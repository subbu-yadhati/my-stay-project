...
const ProductList = ({ products }) => {
  ...
  const toSlug = (text) => {
    ...
    textLower = textLower.replace(/[^a-z0-9\s]/g, '');
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mt-[24px]">
        {products.map((post) => (
          ...
          <article
            ...
            onClick={() => navigate(`/product/${toSlug(post.title)}`)}
          >
            
            {console.log("full post is ", post)}
            {console.log("hello world 123", toSlug(post.title))}
            <img
