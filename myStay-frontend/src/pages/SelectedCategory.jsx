...
const SelectedCategory = () => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      console.log(`successfully getting the posts`)
  ...
  return (
    <div className="flex flex-col items-center w-[92%] p-8 relative bottom-7 mt-11 mx-auto">
      <h1 className="text-2xl font-bold mb-8 bg-gray-200 mt-5 p-4">{category.name}</h1>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {posts.map((post, index) => (
          ...
          >
            ...
            <div
              className="z-10 text-sm bg-blue-200 leading-6 p-2 text-gray-900 font-bold"
              onClick={() => navigate(`/product/${post.slug}`)}
            >
              {post.title}
            </div>
