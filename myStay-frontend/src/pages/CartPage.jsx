...
const CartPage = () => {
  ...
  const totalPrice = () => {
    try {
      return cart.reduce((total, item) => total + item.price, 0).toLocaleString("en-US", {
        style: "currency",
        currency: "USD",
      });
    } catch (error) {
  ...
  const handleCheckIn = () => {
    ...
    navigate("/payment", {
      state: {
        price: totalPrice(), // Send number, not formatted string
        product: cart.length === 1 ? cart[0].title : "Multiple Products",
        postId: cart.length === 1 ? cart[0]._id : cart[0]._id,
        products: cart.map((product) => ({
          title: product.title,
          postId: product._id,
          price: product.price,
    ...
    console.log('post product details will be', cart.map((product) => ({
      title: product.title,
      postId: product._id,
      price: product.price,
    })),)
